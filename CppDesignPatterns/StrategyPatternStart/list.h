...
namespace demo {
	...
	public:
		...
		int length();
		T remove(int index);
		int collect(Predicate<T> & p);
		void transform(Transformation<T> & t);
	private:
		T removeFirst();
		T removeLast();
	...
	template <typename T> int LinkedList<T>::collect(Predicate<T> & p) {
		int total = 0;
		Node<T> * current = first;
		while(current != 0) {
			if(p.match(current->getItem())) {
				total++;
			}
			current = current->getNext();
		}
	...
	template <typename T> void LinkedList<T>::transform(Transformation<T> & t) {
		Node<T> * current = first;
		while(current != 0) {
			t.apply(current->getItem());
			current = current->getNext();
		}
	}
